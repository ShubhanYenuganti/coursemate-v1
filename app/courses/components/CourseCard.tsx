...
const CourseCard: React.FC<CourseCardProps> = ({ course, onTogglePin }) => {
  ...
  const handleButtonClick = (e: React.MouseEvent, tab: string) => {
    e.stopPropagation();
    router.push(`/courses/${course.dbId}?tab=${tab}`);
  };

  return (
    <div 
      onClick={handleCardClick}
      className={`block bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 border border-gray-200 ${course.isPinned ? 'ring-2 ring-blue-200' : ''} group relative overflow-hidden cursor-pointer`}
    >
      {course.isPinned && (
      ...
      <div className="p-6">
        ...
        <div className="flex gap-2 mt-4">
          <button 
            onClick={(e) => handleButtonClick(e, 'study')}
          ...
          <button 
            onClick={(e) => handleButtonClick(e, 'materials')}
            className="flex-1 bg-gray-100 text-gray-800 py-2 px-3 rounded-md text-xs font-medium hover:bg-gray-200 transition-colors flex items-center justify-center gap-1"
          >
            ...
            <button 
            onClick={(e) => handleButtonClick(e, 'ai')}
            className="flex-1 bg-gray-100 text-gray-800 py-2 px-3 rounded-md text-xs font-medium hover:bg-gray-200 transition-colors flex items-center justify-center gap-1"
            >
