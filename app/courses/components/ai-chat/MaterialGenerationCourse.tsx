...
import { BookOpen, Brain, FileText, Download, Loader2, Sparkles, ChevronLeft, ChevronRight, RotateCcw, Grid, Play, Save } from 'lucide-react';
...
export default function MaterialGenerationCourse({ courseId }: MaterialGenerationCourseProps) {
  ...
  const saveQuizToMaterials = async () => {
    ...
    try {
      ...
      console.log('Saving quiz to:', endpoint);
      ...
      if (!response.ok) {
        ...
        console.error('Failed to save quiz. Status:', response.status, 'Error:', errorText);
  ...
  const saveFlashcardsToMaterials = async () => {
    ...
    try {
      ...
      console.log('Saving flashcards to:', endpoint);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Failed to save flashcards. Status:', response.status, 'Error:', errorText);
  ...
  return (
    <div className="w-full h-full">
      <div className="flex flex-col h-[600px] bg-white rounded-lg shadow-sm border mx-6">
