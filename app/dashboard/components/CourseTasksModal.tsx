...
const CourseTasksModal: React.FC<CourseTasksModalProps> = ({ isOpen, onClose, course }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-lg sm:rounded-xl shadow-lg w-full max-w-2xl max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 ${course.iconBg} ${course.iconColor} rounded-lg flex items-center justify-center text-sm sm:text-base`}>
              {course.icon}
            </div>
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-gray-800">{course.name}</h2>
              <p className="text-xs sm:text-sm text-gray-600">{course.professor} • {course.schedule}</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex gap-1 sm:gap-2 overflow-x-auto">
            {[
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-xs sm:text-sm font-medium border transition-colors focus:outline-none whitespace-nowrap ${
                  filter === key 
        ...
        <div className="p-4 sm:p-6 overflow-y-auto max-h-96">
          {isLoading ? (
            ...
            <div className="space-y-2 sm:space-y-3">
              {filteredTasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className={`flex items-center p-3 sm:p-4 border rounded-lg ${
                      overdue ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'
                    }`}
                  >
                    <button
                      onClick={() => handleTaskToggle(task.id, !task.completed)}
                      className="mr-2 sm:mr-3 text-gray-400 hover:text-indigo-500 transition-colors"
                    >
                      {task.completed ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 sm:gap-2 mt-1">
                        ...
                        />
                        <span className={`text-xs ${overdue ? 'text-red-600' : 'text-gray-600'} truncate`}>
                          {task.course}
                        </span>
                        {overdue && (
                          <span className="text-[10px] sm:text-xs bg-red-100 text-red-700 px-1 sm:px-2 py-0.5 rounded-full font-medium whitespace-nowrap">
                            OVERDUE
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 text-xs text-gray-500 ml-2">
                      <Clock className="w-3 h-3" />
                      <span className="hidden sm:inline">{task.time}</span>
                      <span className="sm:hidden">{task.time.substring(0, 5)}</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="text-center py-6 sm:py-8 text-gray-500">
              ...
              <p className="text-base sm:text-lg font-medium mb-1">No tasks found</p>
              <p className="text-sm">
                {filter === 'all' 
