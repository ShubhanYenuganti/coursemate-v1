...
export const groupTasksByTaskId = (goals: Goal[]) => {
  ...
  goals.forEach(goal => {
    if (goal.goal_id === "Google Calendar") {
      ...
      groupedTasks[key] = {
        ...
        hasSubtasks: false // Google Calendar events are treated individually
      };
    } else {
      ...
      if (!groupedTasks[taskId]) {
        groupedTasks[taskId] = {
          ...
          courseId: goal.course_id,
          googleCalendarColor: goal.google_calendar_color ?? null,
          hasSubtasks: false // initialized false, will check later
        };
      }
      ...
      // ✅ Mark hasSubtasks true if any subtask_id is valid
      if (goal.subtask_id !== null && goal.subtask_id !== undefined) {
        groupedTasks[taskId].hasSubtasks = true;
      }
    }
  });
  ...
  Object.values(groupedTasks).forEach(group => {
    ...
    group.progress = group.totalSubtasks > 0
